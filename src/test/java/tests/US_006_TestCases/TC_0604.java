...
public class TC_0604 {
    ...
    @Test
    public void negativeTestNameIle() throws InterruptedException {
    ...
    @Test (dependsOnMethods = "negativeTestNameIle")
    public void negativeTestLocation() throws InterruptedException {
        ...
        actions.sendKeys(Keys.TAB).sendKeys(Keys.CLEAR).sendKeys(faker.address().zipCode()).sendKeys(Keys.TAB)
                .sendKeys(Keys.CLEAR).sendKeys(faker.name().fullName())
                .perform();
        Driver.getDriver().findElement(By.xpath("//input[@name='Location']")).clear();
        Thread.sleep(5000);
